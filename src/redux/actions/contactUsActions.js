...
export const createContactUs = ({name, companyName, phone, email, message, status}) => async (dispatch) => {
  dispatch({ type: CONTACT_US_ACTIONS.CONTACT_US_LOADING})
  try {
    const res = await contactUsServices.createContactUs({name, companyName, phone, email, message, status});
    if(res){
